...
public class Solution {
    ...
    public boolean stoneGame(int[] piles) {
        int times = piles.length / 2;
        return pickAndCheckWin(1, times, 0, 0, piles);
    }

    private boolean pickAndCheckWin(int i, int times, int totalAtlan, int totalLee, int[] piles) {
        if (i > times) {
            return totalAtlan > totalLee;
        }
        if (!pickAndCheckWin(i + 1, times, totalAtlan + piles[i - 1], totalLee + piles[piles.length - i], piles)) {
            return pickAndCheckWin(i + 1, times, totalAtlan + piles[piles.length - i], totalLee + piles[i - 1], piles);
        } else {
            return true;
        }
    }
